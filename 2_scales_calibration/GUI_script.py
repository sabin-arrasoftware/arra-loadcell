...
def display_numbers(scale_num):
    if scales[scale_num]["is_displaying"]:
        ...
        scale_timestamps.append(datetime.now().strftime("%H:%M:%S"))
...
for scale_num in scales:
    ...
    scale_checkbox.grid(row=0, column=scale_num-1, padx=10, pady=10)

    scale_values_text = tk.Text(window, relief="solid", width=20, height=10)
    scale_values_text.grid(row=1, column=scale_num-1, padx=10, pady=10)
